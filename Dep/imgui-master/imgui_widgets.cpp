...
bool ImGui::Checkbox(const char* label, bool* v, bool show_label)
{
    ...
    if (show_label && label_size.x > 0.0f)
...
// FIXME: Rename to BeginListBox()
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items, const ImVec2& _size)
{
    // Size default to hold ~7 items. Fractional number of items helps seeing that we can scroll down/up without looking at scrollbar.
...
bool ImGui::ListBox(const char* label, int* current_item, const char* const items[], int items_count, int height_items, const ImVec2& size)
{
    const bool value_changed = ListBox(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_items, size);
    return value_changed;
}

bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, const ImVec2& size)
{
    if (!ListBoxHeader(label, items_count, height_in_items, size))
        return false;
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size, const std::vector<std::string>& sys_names)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
        ...
        if (hovered)
        {
			...
			else if (plot_type == ImGuiPlotType_Histogram)
			{
				if (sys_names.empty())
					...
					SetTooltip("%s: %8.4g", sys_names[v_idx].c_str(), v0);
			}
            v_hovered = v_idx;
        }
...
IMGUI_API void ImGui::PlotHistogram(const std::vector<std::string>& sys_names, const char * label, const float * values, int values_count, int values_offset, const char * overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride)
{
	ImGuiPlotArrayGetterData data(values, stride);
	PlotEx(ImGuiPlotType_Histogram, label, &Plot_ArrayGetter, (void*)&data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size, sys_names);
}
