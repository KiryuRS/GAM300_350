...
namespace ImGui
{
    ...
    IMGUI_API bool          Checkbox(const char* label, bool* v, bool show_label = false);
    ...
    // Widgets: List Boxes
	IMGUI_API bool          ListBox(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items = -1, const ImVec2& size = ImVec2{ 0,0 });
	IMGUI_API bool          ListBox(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int height_in_items = -1, const ImVec2& size = ImVec2{ 0,0 });
    IMGUI_API bool          ListBoxHeader(const char* label, const ImVec2& size = ImVec2(0,0)); // use if you want to reimplement ListBox() will custom data or interactions. if the function return true, you can output elements then call ListBoxFooter() afterwards.
	IMGUI_API bool          ListBoxHeader(const char* label, int items_count, int height_in_items = -1, const ImVec2& size = ImVec2{ 0,0 }); // "
	...
	IMGUI_API void			PlotHistogram(const std::vector<std::string>& sys_names, const char* label, const float* values, int values_count, int values_offset = 0, const char* overlay_text = NULL, float scale_min = FLT_MAX, float scale_max = FLT_MAX, ImVec2 graph_size = ImVec2(0, 0), int stride = sizeof(float));
